...
def get_interest_points(image, feature_width):
    ...
    suppressHarrisMatrix = harrisMatrix
    ...
    for i in range(feature_width, image.shape[0] - feature_width):
        for j in range(feature_width, image.shape[1] - feature_width):
            localMaxima[i, j] = np.max(suppressHarrisMatrix[i - offset : i + offset, j - offset : j + offset])
            if(localMaxima[i, j] == suppressHarrisMatrix[i, j]) and suppressHarrisMatrix[i, j] >= (globalMaxima * 0.001):
